...
def compute(s: str):
    ...
    #    if valid(WFS, item):
    #        result += sum(item)
    result = process_wf('in', WFS, rng)
    print(result)
    return result

def process_wf(lbl, WFS, rng):
    if lbl == 'R':
        return 0
    if lbl == 'A':
        ranges =  list(map(lambda x: x[1]-x[0]+1, rng))
        return ranges[0]*ranges[1]*ranges[2]*ranges[3]
    wf = WFS[lbl]
    poss = 0
    for cat, sign, th, res in wf[:-1]:
        lo, hi = rng[cat]
        if sign == '<':
            if lo >= th:  # range doesnt satisfy condition'
                continue 
            if hi < th:  # entire range satisfies condition
                return process_wf(res, WFS, rng)
            # part of range satisfies condition
            nrng = rng[:cat] + ((lo, th-1),) + rng[cat+1:]
            rng = rng[:cat] + ((th, hi),) + rng[cat+1:]
            poss += process_wf(res, WFS, nrng)
        elif sign == '>':
            if hi <= th:
                continue
            if lo > th:
                return process_wf(res, WFS, rng)
            nrng = rng[:cat] + ((th+1, hi),) + rng[cat+1:]
            rng = rng[:cat] + ((lo, th),) + rng[cat+1:]
            poss += process_wf(res, WFS, nrng)
    poss += process_wf(wf[-1], WFS, rng)
    return poss
...
def wf_results(wf):
    return tuple(cond[3] for cond in wf[:-1]) + (wf[-1],)
